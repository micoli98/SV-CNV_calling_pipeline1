...
process Prepare_input {
    ...
    output:
    path "ids_table.tsv" into ids 

    script: 
    """
    Rscript /mnt/storageBig8/work/micoli/temporary_stuff/pipe_dsl2/make_ids.R $samples $pubDir 
    """
} 
...
gridss_output.collect()
    .set{ gridss_collected }

process Collect_GRIDSS {
    input:
    tuple normalSample, patient, Bams from gridss_pon
    val ("*") from gridss_collected

    output:
    val ("*") into gridss_files 

    script:
    """
    #ln -s /mnt/storageBig8/work/micoli/051122/${patient}/${normalSample}_calls.vcf /mnt/storageBig8/work/micoli/051122/PoN/${normalSample}_calls.vcf
    """
}
...
process Gripss {
    ...
    tuple sample, normalSample, patient, bamFile, normalBamFile from sample_info_split
    ...
    """
     $java -jar /mnt/storageBig8/work/micoli/pipeline/Scripts_Dependencies/gripss.jar \
        ...
        -vcf /mnt/storageBig8/work/micoli/051122/${patient}/${normalSample}_calls_filtered.vcf \
        -output_dir .
    """
}
...
process Amber {
    ...
    -reference ${normalSample} \
    -reference_bam /mnt/storageBig8/resources/processed_data/HERCULES/WGSbams/${normalSample}.bam \
    -tumor ${sample} \
    -tumor_bam ${bamFile} \
    -output_dir $pubDir/${patient} \
